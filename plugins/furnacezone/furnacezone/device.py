...
class FurnaceZoneDeviceEditor(core.device.DeviceEditor):
    ...
    def createHardwareBox(self):
        ...
        # fsizer.Add(self.purgeCheckbox, 1, wx.EXPAND | wx.ALL | wx.ALIGN_CENTRE_VERTICAL)  # Dona error
        # fsizer.Add(self.purgeCheckbox, 1, wx.EXPAND | wx.ALL)  # Provem un cas
        fsizer.Add(self.purgeCheckbox, 1, wx.ALIGN_CENTRE_VERTICAL)  # Provem un altre cas
        self.purgeSetpointText = wx.TextCtrl(self.control, -1)
        panel = wx.Panel(self.control, -1, style=wx.SIMPLE_BORDER, size=wx.Size(200, -1))
        panel.SetBackgroundColour(wx.Colour(246, 232, 175))
        ...
        txt = staticwraptext.StaticWrapText(notice, -1, messages.get('device.config.purge.notice'), size=wx.Size(-1, 40))
        s = wx.BoxSizer(wx.HORIZONTAL)
        s.Add(txt, 1, wx.EXPAND)
        notice.SetSizer(s)
        notice.SetAutoLayout(True)
        sb = wx.BoxSizer(wx.HORIZONTAL)
        sb.Add(img, 0, wx.EXPAND | wx.ALL, 5)
        sb.Add(notice, 1, wx.EXPAND | wx.ALL, 5)
        panel.SetSizer(sb)
        panel.SetAutoLayout(True)
        fsizer.Add(wx.Panel(self.control, -1), 0, wx.ALIGN_RIGHT)
        fsizer.Add(panel, 1, wx.EXPAND | wx.ALL, 0)
        ...
        ceSizer.Add(wx.StaticText(self.channelErrorMsg, -1, messages.get('conflictinghardware.message')), 1, wx.EXPAND)
    ...
    def createUIBox(self):
        ...
        self.devicePlotColor = colourselect.ColourSelect(self.control, -1, size=wx.Size(60, 20))
