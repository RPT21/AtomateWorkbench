...
class SerialConfigurationSegment(object):
    ...
    def createControl(self, composite):
        ...
        try:
            ...
            self.wordSizeChoice = wx.ComboBox(self.control, -1, choices=list(map(str, CHOICES_BITS)), style=wx.CB_READONLY)
...
class SerialDeviceDriver(et2216e.drivers.DeviceDriver):
    __module__ = __name__
    ...
    def setMode(self, mode):
        if mode not in [MODES]:
            raise Exception('Error, mode %d not allowed as mode' % mode)
        self.modbusChannel.writeWord(199, mode)
    ...
    def initialize(self):
        try:
            ...
            self.port.open()
            self.checkInterrupt()
            self.modbusChannel.configure(self.port, 2)
    ...
    def setManual(self, manual):
        self.modbusChannel.writeBit(2, manual)
    ...
    def lockPanel(self):
        self.modbusChannel.writeWord(DISABLE_KEYS_REGISTER, 1)

    def unlockPanel(self):
        self.modbusChannel.writeWord(DISABLE_KEYS_REGISTER, 0)

    def getTemperature(self):
        return self.modbusChannel.readWords(1, 1)[0]

    def getSoftwareVersion(self):
        """Returns the product type to make sure it's either 2200 or 2400"""
        sid = '%0.4x' % self.modbusChannel.readWords(122, 1)
        return sid

    def setSP1(self, temp):
        self.modbusChannel.writeWord(24, temp)
    ...
    def shutdown(self):
        ...
        self.port.close()
        self.status = et2216e.drivers.STATUS_UNINITIALIZED


et2216e.drivers.registerDriver('serial', SerialDeviceDriver, SerialConfigurationSegment, 'Serial')
