...
import re, wx, logging, poi.utils.staticwraptext, ui.dialog.preferences, validator.messages as messages, validator.constants
...
class PreferencesPage(ui.dialog.preferences.PreferencesPage):
    ...
    def createControl(self, parent):
        ...
        self.values = {}
        label = wx.StaticText(self.control, -1, messages.get(validator.constants.MESSAGE_VALIDATION_DELAY_LABEL))
        self.validatorDelay = wx.TextCtrl(self.control, -1)
        self.validatorDelay.SetMaxLength(len('000000'))
        helplabel = poi.utils.staticwraptext.StaticWrapText(self.control, -1, messages.get(validator.constants.MESSAGE_VALIDATION_DELAY_HELP))
        ...
        defaultsButton = wx.Button(self.control, -1, messages.get(validator.constants.MESSAGE_RESTORE_DEFAULTS_LABEL))
        mainsizer = wx.BoxSizer(wx.VERTICAL)
        mainsizer.Add(fsizer, 0, wx.GROW)
        mainsizer.Add(helplabel, 0, wx.GROW | wx.LEFT | wx.RIGHT | wx.TOP, 10)
    ...
    def validateFields(self):
        ...
        if len(text) == 0:
            self.setError(messages.get(validator.constants.MESSAGE_ERROR_EMPTY_FIELD))
        ...
        if m:
            self.setError(messages.get(validator.constants.MESSAGE_ERROR_NON_NUMERIC_FIELD))
        else:
            self.setError(None)
        self.update()
        return
    ...
    def getTitle(self):
        return messages.get(validator.constants.MESSAGE_VALIDATOR_PREFERENCES_TITLE)
