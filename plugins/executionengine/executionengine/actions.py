...
import poi.actions, executionengine.messages as messages, executionengine, executionengine.images as images, ui.context, copy, wx, resources, executionengine.purgemanager
isValid = False
oldValidValue = True
...
def abortRecipe():
    executionengine.getDefault().getEngine().stop()


def startRecipe():
    ui.context.setProperty('execution', 'running')
    ui.context.setProperty('can-edit', False)
    recipe = ui.context.getProperty('recipe')
    engine = executionengine.getDefault().createEngine()
...
def updateActions():
    ...
    executionengine.getDefault().fireEnablementState()
...
class RunRecipeAction(poi.actions.Action):
    __module__ = __name__

    def __init__(self):
        poi.actions.Action.__init__(self, messages.get('runmenu.run'), messages.get('runmenu.run.tip'), messages.get('runmenu.run.tip'))
...
class PauseRecipeAction(poi.actions.Action):
    __module__ = __name__

    def __init__(self):
        poi.actions.Action.__init__(self, messages.get('runmenu.pause'), messages.get('runmenu.pause.tip'), messages.get('runmenu.pause.tip'))
        self.setEnabled(False)
        self.setImage(images.getImage(images.PAUSE_ENABLED))
        self.setDisabledImage(images.getImage(images.PAUSE_DISABLED))

    def run(self):
        ui.context.setProperty('execution', 'paused')
        executionengine.getDefault().getEngine().pause()


class ResumeRecipeAction(poi.actions.Action):
    __module__ = __name__

    def __init__(self):
        poi.actions.Action.__init__(self, messages.get('runmenu.resume'), messages.get('runmenu.resume.tip'), messages.get('runmenu.resume.tip'))
        self.setEnabled(False)
    ...
    def run(self):
        ui.context.setProperty('execution', 'running')
        executionengine.getDefault().getEngine().resume()


class AdvanceRecipeAction(poi.actions.Action):
    __module__ = __name__

    def __init__(self):
        poi.actions.Action.__init__(self, messages.get('runmenu.advance'), messages.get('runmenu.advance.tip'), messages.get('runmenu.advance.tip'))
        self.setEnabled(False)
    ...
    def run(self):
        ui.context.setProperty('execution', 'running')
        executionengine.getDefault().getEngine().advance()


class AbortRecipeAction(poi.actions.Action):
    __module__ = __name__

    def __init__(self):
        poi.actions.Action.__init__(self, messages.get('runmenu.abort'), messages.get('runmenu.abort.tip'), messages.get('runmenu.abort.tip'))
        self.setEnabled(False)
...
class StopPurgeAction(poi.actions.Action):
    __module__ = __name__

    def __init__(self):
        poi.actions.Action.__init__(self, messages.get('runmenu.stoppurge'), messages.get('runmenu.stoppurge.tip'), messages.get('runmenu.stoppurge.tip'))
        self.setEnabled(False)
        self.setImage(images.getImage(images.STOP_PURGE_ENABLED))
        self.setDisabledImage(images.getImage(images.STOP_PURGE_DISABLED))

    def run(self):
        executionengine.purgemanager.cancelPurge()
