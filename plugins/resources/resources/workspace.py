...
import os, sys, shutil, kernel, resources.utils, resources.project, resources.version, resources.runlog, traceback, logging
...
def getProjectsAtPath(location):
    ...
    for filename in os.listdir(location):
        ...
        if not os.path.isdir(fullpath):
            continue
        if not resources.project.isProject(fullpath):
            continue
        ...
        try:
            ...
            logger.exception(msg)
            logger.warn("Could not load project '%s': '%s'" % (filename, msg))
...
def getRecipeVersions(project):
    ...
    for filename in os.listdir(location):
        ...
        if not os.path.isdir(fullpath):
            continue
        if not resources.version.isRecipeVersion(fullpath):
            continue
        version = resources.version.RecipeVersion(filename)
...
def getRunLogs(version):
    ...
    for filename in os.listdir(location):
        ...
        if not os.path.isfile(fullpath):
            continue
        if not resources.runlog.isRunLog(fullpath):
            continue
        runlog = resources.runlog.RunLog(os.path.splitext(filename)[0])
...
def getRecipeVersion(project, name):
    location = project.getLocation()
    newversion = resources.version.RecipeVersion(name)
...
def getRunLog(recipeVersion, runVersion):
    newRunLog = resources.runlog.RunLog(str(runVersion))
...
def setLocalPrefix(prefix):
    global localprefix
    if prefix is None:
        localprefix = resources.utils.getUsersDirectory()
    else:
        localprefix = prefix
    return
...
def getSharedLocation():
    return os.path.join(kernel.getSite(), WORKSPACE_DIRNAME)
...
def getProject(name, shared=False):
    ...
    project = resources.project.Project(name)
    project.setLocation(location)
    project.internal_setParent(resources.getDefault().getWorkspace())
    return project
