...
import kernel, os, sys, resources, core.recipe, core.utils
...
class RecipeVersion(resources.Resource):
    ...
    def create(self):
        ...
        resources.getDefault().getWorkspace().fireWorkspaceChangeEvent(resources.workspace.WorkspaceChangeEvent(resources.workspace.TYPE_CREATE, self))

    def createRecipeDataFile(self):
        name = self.getRecipeDataFilename()
        try:
            f = open(name, 'w')
            recipe = core.recipe.Recipe()
            node = core.recipe.convertToNode(recipe)
