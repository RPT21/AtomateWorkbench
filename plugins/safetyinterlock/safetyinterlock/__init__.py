...
import wx, os, hw, executionengine, ui, ui.images as uiimages, kernel.plugin, poi.actions, safetyinterlock.actions, poi.actions, poi.views, poi.views.viewers, poi.views.contentprovider, hardware.hardwaremanager, safetyinterlock.images as images, safetyinterlock.messages as messages, logging
instance = None
...
class SafetyInterlockPlugin(kernel.plugin.Plugin):
    ...
    def setStatus(self, status):
        ...
        if not self.ready:
            return
        executionengine.getDefault().updateEnablementState()
...
class StatusDialog(wx.Dialog):
    ...
    def __init__(self):
        wx.Dialog.__init__(self, None, -1, 'Safety Interlock Status', size=(400, 400), style=wx.DEFAULT_DIALOG_STYLE | wx.RESIZE_BORDER)
        ...
        self.mainStatusPanel = wx.Panel(self, -1, size=(-1, 40), style=wx.SUNKEN_BORDER)
        ...
        sizer.Add(self.mainStatusPanel, 0, wx.GROW | wx.ALL, 3)
        sizer.Add(control, 1, wx.GROW | wx.ALL, 5)
        ...
        sizer.Add(control, 1, wx.GROW | wx.LEFT | wx.RIGHT | wx.BOTTOM, 5)
        sizer.Add(wx.StaticLine(self, -1), 0, wx.GROW)
...
class ShowSafetyLockStatusAction(poi.actions.Action):
    ...
    def run(self):
        safetyinterlock.getDefault().toggleStatusDialog()
