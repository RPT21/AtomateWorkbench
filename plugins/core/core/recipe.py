...
import traceback, core.recipestep, core.configurationelement, core.device, copy, logging
...
def loadFromString(buff):
    root = core.configurationelement.loadFromString(buff)
    if root is None:
    ...
    for deviceNode in devicesNode.getChildren('device'):
        ...
        if not deviceID:
            deviceID = core.device.createDeviceID()
            logger.warn('No device id specified, using new %s' % deviceID)
        logger.debug('Using device id: %s' % deviceID)
        inst = findDeviceInRegistry(deviceType)
        if inst is None:
            logger.warn("No devuce type found for name '%s'" % deviceType)
            continue
    ...
    for stepNode in stepNodes.getChildren('step'):
        try:
            step = core.recipestep.fromNode(stepNode, recipe)
            recipe.addStep(step)
        except Exception, msg:
            logger.exception(msg)
...
def convertToNode(recipe):
    node = core.configurationelement.new('recipe')
