...
import copy, wx, string, ui, ui.messages as messages, poi.actions, poi.views, logging, traceback
...
class Sector(object):
    ...
    def createControl(self, parent, sid, id, defaultSize, style, orientation, alignment, visSash=None):
        ...
        if visSash is not None:
            self.control = wx.SashLayoutWindow(parent, id=id, size=defaultSize, style=style)
            ...
            self.control = poi.views.OneChildWindow(parent, -1)
    ...
    def createStage(self):
        self.stage = poi.views.OneChildWindow(self.control, -1)
...
class MainFrame(object):
    ...
    def addPerspective(self, id, perspective):
        if self.perspectives.has_key(id):
            return
        self.perspectives[id] = perspective

    def getPerspective(self, id):
        if not self.perspectives.has_key(id):
    ...
    def addMenuContributions(self):
        ...
        class HideAction(poi.actions.Action):
            __module__ = __name__

            def __init__(self):
                poi.actions.Action.__init__(self, 'Debug Hide Sectors', 'Debug Hide Sectors', 'Debug Hide Sectors', style=poi.actions.AS_CHECK_BOX)
        ...
        viewsManager.addItem(poi.actions.ActionContributionItem(self.hideAction))
    ...
    def createView(self, sectorID, provider, viewID):
        ...
        if not self.sectors.has_key(sectorID):
    ...
    def removeView(self, viewID):
        if not self.views.has_key(viewID):
            return
    ...
    def clearSector(self, sectorID):
        if self.sectors2Views.has_key(sectorID):
            ...
            if hasattr(child, 'dispose'):
                ...
                logger.warn('Child of sector does not have dispose method: %s' % child)
    ...
    def createSashLayoutSectors(self):
        ...
        self.stageParts = {'north': {'id': 1000, 'size': (100, 30), 'style': 0, 'orientation': (wx.LAYOUT_HORIZONTAL), 'alignment': (wx.LAYOUT_TOP), 'sash': (wx.SASH_BOTTOM)}, 'south': {'id': 1001, 'size': (100, 100), 'style': 0, 'orientation': (wx.LAYOUT_HORIZONTAL), 'alignment': (wx.LAYOUT_BOTTOM), 'sash': (wx.SASH_TOP)}, 'west': {'id': 1002, 'size': (100, 30), 'style': 0, 'orientation': (wx.LAYOUT_VERTICAL), 'alignment': (wx.LAYOUT_BOTTOM), 'sash': (wx.SASH_TOP)}, 'east': {'id': 1003, 'size': (100, 30), 'style': 0, 'orientation': (wx.LAYOUT_HORIZONTAL), 'alignment': (wx.LAYOUT_TOP), 'sash': (wx.SASH_TOP)}}
        south = Sector()
        south.createControl(self.sectorsView, 'south', 1001, (100, 100), 0, wx.LAYOUT_HORIZONTAL, wx.LAYOUT_BOTTOM, wx.SASH_TOP)
        west = Sector()
        west.createControl(self.sectorsView, 'west', 1002, (100, 30), 0, wx.LAYOUT_VERTICAL, wx.LAYOUT_LEFT, wx.SASH_RIGHT)
        east = Sector()
        east.createControl(self.sectorsView, 'east', 1003, (100, 100), 0, wx.LAYOUT_VERTICAL, wx.LAYOUT_RIGHT, wx.SASH_LEFT)
        center = Sector()
        center.createControl(self.sectorsView, 'center', 1000, (100, 30), 0, wx.LAYOUT_VERTICAL, wx.LAYOUT_LEFT)
        ...
        def draggedSash(event):
            ...
            ydir = [
             wx.SASH_BOTTOM, wx.SASH_TOP]
            xdir = [wx.SASH_LEFT, wx.SASH_RIGHT]
        ...
        wx.EVT_SASH_DRAGGED_RANGE(self.sectorsView, 1000, 1003, draggedSash)
    ...
    def restoreLayout(self, memento):
        ...
        try:
            ...
            self.control.SetPosition((int(dims[0]), int(dims[1])))
            self.control.SetSize((int(dims[2]), int(dims[3])))
            ...
            for (sectorName, window) in self.sectors.items():
                strsize = memento.get('mainframe.layout', string.join(['sector', sectorName], '.'))
                size = map(int, strsize.split(','))
                if size[0] < 20:
                    size[0] = 20
                if size[1] < 20:
                    size[1] = 20
            ...
            logger.warn("Unable to recreate from memento, defaulting: '%s'" % msg)
            self.createDefaultLayout()
    ...
    def saveLayout(self, memento):
        ...
        for (sectorName, window) in self.sectors.items():
            ...
            memento.set('mainframe.layout', string.join(['sector', sectorName], '.'), '%s,%s' % tuple(map(str, size)))
