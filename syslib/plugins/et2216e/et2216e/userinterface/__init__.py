...
import validator, wx, ui, time, string, logging, poi.views, core.utils, poi.dialogs, hardware.userinterface.configurator, hardware.hardwaremanager, et2216e.drivers, et2216e.userinterface.initdialog, et2216e.messages as messages, threading, poi.operation, poi.dialogs.progress, wx.lib.colourselect as colourselect
...
logger = logging.getLogger('et2216e.ui')
...
class ConfigurationPage(hardware.userinterface.configurator.ConfigurationPage):
    ...
    def createConfigPanel(self, parent):
        ...
        self.rangeText = wx.TextCtrl(panel, -1, '')
        fsizer.Add(wx.StaticText(panel, -1, messages.get('dialog.hwconfig.default.device.range.label')), 0, wx.ALIGN_RIGHT | wx.ALIGN_CENTRE_VERTICAL)
        fsizer.Add(self.rangeText, 0, wx.ALIGN_LEFT | wx.ALIGN_CENTRE_VERTICAL)
        panel.Bind(wx.EVT_TEXT, self.markDirty, self.rangeText)
    ...
    def OnDriverChoice(self, event):
        ...
        page = et2216e.drivers.getDriverPageByName(choice)
    ...
    def getDriverOptions(self):
        keys = et2216e.drivers.getRegisteredDeviceKeys()
        names = []
        for key in keys:
            names.append(et2216e.drivers.getDriverName(key))
    ...
    def setData(self, config):
        ...
        if config.has_section('default.device.props'):
            try:
                ...
                tv = 1000
                try:
                    tv = int(config.get('default.device.props', 'range'))
                except Exception, ignored:
                    logger.exception(ignored)

                self.rangeText.SetValue(str(tv))
            except Exception, msg:
                logger.exception(msg)
    ...
    def setDriverConfig(self, config):
        ...
        driverName = et2216e.drivers.getDriverName(driverType)
        options = self.getDriverOptions()
        idx = options.index(driverName)
        self.driverCombo.SetSelection(idx)
        page = et2216e.drivers.getDriverConfigurationPage(driverType)
        self.updateDriverSegment(page)
        page.setData(config)
    ...
    def getData(self, config):
        ...
        driverType = et2216e.drivers.getDriverTypeByName(self.driverCombo.GetStringSelection())
        ...
        tv = 1000
        try:
            tv = int(self.rangeText.GetValue())
        except Exception, ignored:
            logger.exception(ignored)

        config.set('default.device.props', 'range', str(tv))
        return
