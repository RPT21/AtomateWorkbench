...
import os, traceback, wx, string, logging, core.utils, poi.views, poi.dialogs, hardware.userinterface.configurator, hardware.hardwaremanager, adr2100.drivers, threading, ui, poi.operation, time, poi.dialogs.progress, mfc.messages as messages, ui.images as uiimages, poi.utils.staticwraptext as staticwraptext, ui.images as uiimages, compiler
...
class ConfigurationPage(hardware.userinterface.configurator.ConfigurationPage):
    ...
    def createConfigPanel(self, parent):
        ...
        boxSizer.Add(inbox, 0, wx.GROW | wx.ALL, 2)
        boxSizer.Add(self.requestCodeText, 1, wx.GROW | wx.ALL, 5)
        hsizer = wx.BoxSizer(wx.HORIZONTAL)
        hsizer.Add(self.requestCodeErrorIcon, 0, wx.ALL | wx.ALIGN_CENTRE_VERTICAL, 1)
        hsizer.Add(self.requestCodeErrorText, 1, wx.ALL | wx.GROW, 1)
        boxSizer.Add(hsizer, 0, wx.GROW | wx.ALL, 2)
        inbox = wx.Panel(panel, -1, style=wx.SIMPLE_BORDER)
        inbox.SetBackgroundColour(wx.Color(246, 232, 175))
        ...
        boxSizer.Add(inbox, 0, wx.GROW | wx.ALL, 2)
        boxSizer.Add(self.validationCodeText, 1, wx.GROW | wx.ALL, 5)
        hsizer = wx.BoxSizer(wx.HORIZONTAL)
        hsizer.Add(self.validationCodeErrorIcon, 0, wx.ALL | wx.ALIGN_CENTRE_VERTICAL, 1)
        hsizer.Add(self.validationCodeErrorText, 1, wx.ALL | wx.GROW, 1)
        boxSizer.Add(hsizer, 0, wx.GROW | wx.ALL, 2)
    ...
    def compileString(self, codeStr, name):
        try:
            code = compiler.compile(codeStr, name, 'exec')
            return (True, code)
    ...
    def OnDriverChoice(self, event):
        ...
        page = adr2100.drivers.getDriverPageByName(choice)
    ...
    def getDriverOptions(self):
        keys = adr2100.drivers.getRegisteredDeviceKeys()
        names = []
        for key in keys:
            names.append(adr2100.drivers.getDriverName(key))
    ...
    def setDriverConfig(self, config):
        ...
        driverName = adr2100.drivers.getDriverName(driverType)
        options = self.getDriverOptions()
        idx = options.index(driverName)
        self.driverCombo.SetSelection(idx)
        page = adr2100.drivers.getDriverConfigurationPage(driverType)
    ...
    def getData(self, config):
        ...
        driverType = adr2100.drivers.getDriverTypeByName(self.driverCombo.GetStringSelection())
    ...
    def initializeHardware(self):
        ...
        try:
            ...
            poi.dialogs.ExceptionDialog(f, invocation, 'Error Initializing Hardware').ShowModal()
    ...
    def shutdownHardware(self):
        ...
        try:
            ...
            logger.exception(msg)
            poi.dialogs.ExceptionDialog(f, str(msg), 'Error Shutting Down Hardware').ShowModal()
